...
def start_text(name: str) -> str:
    return (
        f"{te('tag','🏷️')} <b>TAG MASTER BOT</b> {te('tag','🏷️')}\n\n"
...
async def cmd_help(client: Client, message: Message) -> None:
    ...
    if not result:
        await message.reply_text(text, reply_markup=_fallback_back_kb())
...
async def callback_handler(client: Client, query: CallbackQuery) -> None:
    ...
    if data == "cb_help":
        ...
        if not result:
            await query.message.edit_text(text, reply_markup=_fallback_back_kb())
